...
int main(void)
{
  ...
  TxHeader.StdId = 0x103; // the standard ID is 0x446

  //define the data that we will send
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  ...
	  if (datacheck) // if the flag is set, then blink the LED
	  {
		  ...
		  // after the LED is finished blinking, we will send the data
...
static void MX_CAN1_Init(void)
{
  ...
  canfilterconfig.FilterFIFOAssignment = CAN_FILTER_FIFO1; //use FIFO1 to store the incoming message
  canfilterconfig.FilterIdHigh = 0x446<<5; //this is the ID that should pass through, this ID will be assigned to blue pill
  canfilterconfig.FilterIdLow = 0;
  canfilterconfig.FilterMaskIdHigh = 0x446<<5; //the same value in the Master ID register also
